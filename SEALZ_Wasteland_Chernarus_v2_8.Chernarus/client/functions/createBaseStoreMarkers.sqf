...
waitUntil {{!isNull(missionNamespace getVariable _x) && ((getPos(missionNamespace getVariable _x) distance [0,0,0]) > 100)} count _baseStores == count _baseStores};
{
	...
	_markerName setMarkerTextLocal "Base Supply Store";
    ...
    _markerName setMarkerColorLocal _col_empty;
	_markerName setMarkerSizeLocal [0.8,0.8];
	_markerName setMarkerTextLocal "Base Supply Store is Empty";
    _markerName setMarkerAlphaLocal 0.5;
...
_setStatus = {
	...
	_markerNameZone = format ["marker_shop_zone_%1", _baseStores select (_this select 0)];
	_markerNameDescription = format ["marker_shop_desc_%1", _baseStores select (_this select 0)];
	switch(_this select 1) do {
		case "EMPTY": {
			_markerNameZone setmarkerColorLocal _col_empty;
			_markerNameDescription setmarkerColorLocal _col_empty;
			_markerNameDescription setMarkerTextLocal "Base Supply Store is Empty";
		};
		case "ENEMY": {
			_markerNameZone setmarkerColorLocal _col_enemy;
			_markerNameDescription setmarkerColorLocal _col_enemy;
			_markerNameDescription setMarkerTextLocal "Base Supply Store has Enemy Players";
		};
		case "FRIENDLY": {
			_markerNameZone setmarkerColorLocal _col_friendly;
			_markerNameDescription setmarkerColorLocal _col_friendly;
			_markerNameDescription setMarkerTextLocal "Base Supply Store has Friendly Players";
		};
		case "MIXED": {
			_markerNameZone setmarkerColorLocal _col_mixed;
			_markerNameDescription setmarkerColorLocal _col_mixed;
			_markerNameDescription setMarkerTextLocal "Base Supply Store has Enemy and Friendly Players";
		};
	};
...
while {showmarkers} do {
    {
    	_unit = missionNamespace getVariable (_baseStores select _forEachIndex);
		_friendlyCount = 0;
		_enemyCount = 0;
		{
