//	@file Version: 1.0
//	@file Name: createheliStoreMarkers.sqf
...
waitUntil {{!isNull(missionNamespace getVariable _x) && ((getPos(missionNamespace getVariable _x) distance [0,0,0]) > 100)} count _heliStores == count _heliStores};
{
	...
	_markerName setMarkerTextLocal "Heli Store";

	// heli store description    
    ...
    _markerName setMarkerColorLocal _col_empty;
	_markerName setMarkerSizeLocal [0.8,0.8];
	_markerName setMarkerTextLocal "Heli Store is Empty";
    _markerName setMarkerAlphaLocal 0.5;
...
_setStatus = {
	...
	_markerNameZone = format ["marker_shop_zone_%1", _heliStores select (_this select 0)];
	_markerNameDescription = format ["marker_shop_desc_%1", _heliStores select (_this select 0)];
	switch(_this select 1) do {
		case "EMPTY": {
			_markerNameZone setmarkerColorLocal _col_empty;
			_markerNameDescription setmarkerColorLocal _col_empty;
			_markerNameDescription setMarkerTextLocal "Heli Store is Empty";
		};
		case "ENEMY": {
			_markerNameZone setmarkerColorLocal _col_enemy;
			_markerNameDescription setmarkerColorLocal _col_enemy;
			_markerNameDescription setMarkerTextLocal "Heli Store has Enemy Players";
		};
		case "FRIENDLY": {
			_markerNameZone setmarkerColorLocal _col_friendly;
			_markerNameDescription setmarkerColorLocal _col_friendly;
			_markerNameDescription setMarkerTextLocal "Heli Store has Friendly Players";
		};
		case "MIXED": {
			_markerNameZone setmarkerColorLocal _col_mixed;
			_markerNameDescription setmarkerColorLocal _col_mixed;
			_markerNameDescription setMarkerTextLocal "Heli Store has Enemy and Friendly Players";
		};
	};
...
while {showmarkers} do {
    {
    	_unit = missionNamespace getVariable (_heliStores select _forEachIndex);
		_friendlyCount = 0;
		_enemyCount = 0;
		{
