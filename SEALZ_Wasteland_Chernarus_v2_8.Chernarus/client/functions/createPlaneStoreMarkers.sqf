//	@file Version: 1.0
//	@file Name: createplaneStoreMarkers.sqf
...
waitUntil {{!isNull(missionNamespace getVariable _x) && ((getPos(missionNamespace getVariable _x) distance [0,0,0]) > 100)} count _planeStores == count _planeStores};
{
	...
	_markerName setMarkerTextLocal "Plane Store";

	// plane store description    
...
while {showmarkers} do {
    {
    	_unit = missionNamespace getVariable (_planeStores select _forEachIndex);
		_friendlyCount = 0;
		_enemyCount = 0;
		{
