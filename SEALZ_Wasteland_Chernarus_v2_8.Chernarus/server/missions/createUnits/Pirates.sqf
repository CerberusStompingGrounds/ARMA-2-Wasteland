...
_leader setskill 1;
_leader addEventHandler["killed", {[_this select 0, _this select 1, "pirates",6000] execVM "Scripts\AIKilled.sqf"}];
[_leader] join _smallGrpS;

//assisstant
_man2 = _smallGrpS createunit ["GUE_Soldier_MG", [(_pos select 0) - 5, _pos select 1, 8], [], 0.5, "Form"];
_man2 setskill 1;
_man2 addEventHandler["killed", {[_this select 0, _this select 1, "pirates",1000] execVM "scripts\AIKilled.sqf"}];
...
sleep 1;
_leader setVehicleInit "nul=[_leader,'Pirate ($6k)'] execVM 'scripts\pirateTracker.sqf';";
sleep 1;
_man2 setVehicleInit "nul=[_man2,'Pirate ($1k)'] execVM 'scripts\pirateTracker.sqf';";
sleep 1;
