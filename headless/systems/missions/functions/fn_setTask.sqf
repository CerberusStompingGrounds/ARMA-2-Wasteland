...
//--- Register
_params = [_this,0,"",["",[]]] call BL_fnc_param;


//--- Get the task ID
if (typeName _params == typeName "") then {_params = [_params, ""]};
_taskID = [_params,0,"",[""]] call BL_fnc_paramIn;
_taskParentID = [_params,1,"",[""]] call BL_fnc_paramIn;
...
_target = [_this,1,_data select 1,[true,sideunknown,grpnull,objnull,[]]] call BL_fnc_param;
...
//--- Set description
_desc = +[_this,2,_data select 2,[[],""],3] call BL_fnc_param;

for "_i" from 0 to 2 do {
	private ["_text","_string"];
	_text = [_desc,_i,[],["",[]]] call BL_fnc_paramin;
	if (typename _text != typename []) then {_text = [_text];};
	_string = [_text,0,"",[""]] call BL_fnc_paramin;
	_text set [0,_string];
	_desc set [_i,_text];
};
_data set [2,_desc];
...
_dest = [_this,3,_data select 3,[objnull,[]]] call BL_fnc_param;
_destTarget = [_dest,0,objnull,[objnull,0]] call BL_fnc_paramin;
if (typename _destTarget == typename objnull) then {
	_dest = [
		_destTarget,
		[_dest,1,false,[false]] call BL_fnc_paramin
	]
} else {
	_dest = [
		_destTarget,
		[_dest,1,0,[0]] call BL_fnc_paramin,
		[_dest,2,0,[0]] call BL_fnc_paramin
	]
};
_data set [3,_dest];
...
//--- Set state
_state = [_this,4,_data select 4,["",true]] call BL_fnc_param;
...
if (_isGlobal && ismultiplayer) then {
	BL_PVAR_currentTasks = missionNamespace getVariable ['BL_PVAR_currentTasks', [] call BL_fnc_hashCreate];
	[BL_PVAR_currentTasks, _taskVar, _data] call BL_fnc_hashSet;
