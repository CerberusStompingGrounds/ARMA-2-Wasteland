...
_base1 = ["AirCraft_Spawn3"] call BIS_fnc_selectRandom;		
_base2 = ["AirCraft_Spawn3_1"] call BIS_fnc_selectRandom;	

	_smallbase1 = ["AirCraft_Spawn3_3"] call BIS_fnc_selectRandom;
	_smallbase2 = ["AirCraft_Spawn3_1_1"] call BIS_fnc_selectRandom;
	_smallbase3 = ["AirCraft_Spawn3_1_2"] call BIS_fnc_selectRandom;
	_smallbase4 = ["AirCraft_Spawn3_2"] call BIS_fnc_selectRandom;
...
_hint = parseText format ["<t align='center' color='%2' shadow='2' size='1.75'>=SEALZ= Air Warfare!</t><br/><t align='center' color='%2'>------------------------------</t><br/><t color='%3' size='1.0'>An Aircraft Mission will spawn in the Balota Airfield in %1 Minutes</t>", _missionDelayTime / 60, _mainTextColour, _subTextColour];
...
_hint = parseText format ["<t align='center' color='%4' shadow='2' size='1.75'>=SEALZ= Air Warfare!</t><br/><t align='center' color='%4'>------------------------------</t><br/><t align='center' color='%5' size='1.25'>%1</t><br/><t align='center'><img size='5' image='%2'/></t><br/><t align='center' color='%5'>A<t color='%4'> %3</t>, will be rewarded to you after you have liberated the enemy AI stronghold at the SW (Balota) Airfield. Reports state that there are 20 well armed enemies. GOOD LUCK!</t>", _missionType, _picture, _vehicleName, _mainTextColour, _subTextColour];
[nil,nil,rHINT,_hint] call RE;

//Add marker to client marker array.
_missionName = format ["%1 Mission",_vehicleName];
clientMissionMarkers set [count clientMissionMarkers,["Aircraft_Marker2",_randomPos,_missionName, "ColorRed"]];
...
sleep 10;
_eastGrpU = createGroup civilian;
[_eastGrpU,_randomPos]execVM "server\missions\createUnits\largeGroup.sqf";
sleep 10;
_eastGrpV = createGroup civilian;
[_eastGrpV,_randomPos2]execVM "server\missions\createUnits\largeGroup.sqf";
...
waitUntil
{
    ...
    _unitsAlive = ({alive _x} count units _eastGrpU);
    _unitsAlive2 = ({alive _x} count units _eastGrpV);	
	_unitsAliveTotal = (_unitsAlive) + (_unitsAlive2);
	[_randomPos,_unitsAliveTotal,"Aircraft_Marker22","ColorRed","20"]execVM "scripts\MissionTracker.sqf";		
...
{
	//Mission Failed.
	_PosDelete = getMarkerPos ("MainBalotaPlaneSpawn"); //Get the position to delete objects
	{deleteVehicle _x} forEach nearestObjects [_PosDelete, _objtoRemoveArray, 250]; //delete all the base objects
    {deleteVehicle _x;}forEach units _eastGrpU; //delete the first group units
    deleteGroup _eastGrpU; //delete the group container
    {deleteVehicle _x;}forEach units _eastGrpV; //delete the second group
    deleteGroup _eastGrpV;  //delete the group container
	...
	//Mission Complete.
    _hint = parseText format ["<t align='center' color='%4' shadow='2' size='1.75'>=SEALZ= Air Warfare!</t><br/><t align='center' color='%4'>------------------------------</t><br/><t align='center' color='%5' size='1.25'>%1</t><br/><t align='center'><img size='5' image='%2'/></t><br/><t align='center' color='%5'>The Balota Airfield AirCraft has been claimed. Nice Job!</t>", _missionType, _picture, _vehicleName, _successTextColour, _subTextColour];
	[nil,nil,rHINT,_hint] call RE;
	sleep 10;
	
    {deleteVehicle _x;}forEach units _eastGrpU;	//delete the first group
	deleteGroup _eastGrpU; // delete the group container
    {deleteVehicle _x;}forEach units _eastGrpV;	//delete the second group
	deleteGroup _eastGrpV;  //delete the group container
	_PosDelete = getMarkerPos ("MainBalotaPlaneSpawn"); // get the position to delete objects
	{deleteVehicle _x} forEach nearestObjects [_PosDelete, _objtoRemoveArray, 250]; //delete all the base objects
	sleep 2;
	_randomPosPlane = getMarkerPos ("MainBalotaPlaneSpawn");	
...
{
    if(_x select 0 == "Aircraft_Marker2") then
    {
