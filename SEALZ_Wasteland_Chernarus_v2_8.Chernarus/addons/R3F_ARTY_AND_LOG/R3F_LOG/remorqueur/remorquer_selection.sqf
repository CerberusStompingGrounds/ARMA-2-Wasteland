...
else
{
	...
	if (!(isNull _objet) && (alive _objet) && !(_objet getVariable "R3F_LOG_disabled")) then
	{
		if (isNull (_objet getVariable "R3F_LOG_est_transporte_par") && (isNull (_objet getVariable "R3F_LOG_est_deplace_par") || (!alive (_objet getVariable "R3F_LOG_est_deplace_par")))) then
		{
			...
			{
				_tempobj = _remorqueur;		_cnt1 = 1;
				while{!isNull(_tempobj getVariable["R3F_LOG_est_transporte_par", objNull])} do {_cnt1 = _cnt1 + 1; _tempobj = _tempobj getVariable["R3F_LOG_est_transporte_par", objNull];};

				_tempobj = _objet;		_cnt2 = 1;
				while{!isNull(_tempobj getVariable["R3F_LOG_remorque", objNull])} do {_cnt2 = _cnt2 + 1; _tempobj = _tempobj getVariable["R3F_LOG_remorque", objNull];};

				if(_cnt1 + _cnt2 <= 4) then {
					// On mémorise sur le réseau que le véhicule remorque quelque chose
					_remorqueur setVariable ["R3F_LOG_remorque", _objet, true]; //ê ÷åìó, êóäà ïðèâûçûâàþò
					// On mémorise aussi sur le réseau que le canon est attaché en remorque
					_objet setVariable ["R3F_LOG_est_transporte_par", _remorqueur, true]; //ê ïðèâÿçûâàåìîìó
					...
					player globalChat "Forget it - 4 vehicles is MAX allowed";
				};
			}
			else
			{
