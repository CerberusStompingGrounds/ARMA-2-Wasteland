//	@file Version: 1.0
//	@file Name: createbountyStoreMarkers.sqf
...
waitUntil {{!isNull(missionNamespace getVariable _x) && ((getPos(missionNamespace getVariable _x) distance [0,0,0]) > 100)} count _bountyStores == count _bountyStores};
{
	...
	_markerName setMarkerTextLocal "Bounty Hunter Store";

	// bounty store description    
    ...
    _markerName setMarkerColorLocal _col_empty;
	_markerName setMarkerSizeLocal [0.8,0.8];
	_markerName setMarkerTextLocal "Bounty Hunter is Empty";
    _markerName setMarkerAlphaLocal 0.5;

	_status set [count _status, "EMPTY"];
} forEach _bountyStores;

//Used to set the status of each store.
_setStatus = {
	if(_status select (_this select 0) == (_this select 1)) exitWith {};

	_markerNameZone = format ["marker_shop_zone_%1", _bountyStores select (_this select 0)];
	_markerNameDescription = format ["marker_shop_desc_%1", _bountyStores select (_this select 0)];
	switch(_this select 1) do {
		case "EMPTY": {
			_markerNameZone setmarkerColorLocal _col_empty;
			_markerNameDescription setmarkerColorLocal _col_empty;
			_markerNameDescription setMarkerTextLocal "Bounty Hunter is Empty";
		};
		case "ENEMY": {
			_markerNameZone setmarkerColorLocal _col_enemy;
			_markerNameDescription setmarkerColorLocal _col_enemy;
			_markerNameDescription setMarkerTextLocal "Bounty Hunter has Enemy Players";
		};
		case "FRIENDLY": {
			_markerNameZone setmarkerColorLocal _col_friendly;
			_markerNameDescription setmarkerColorLocal _col_friendly;
			_markerNameDescription setMarkerTextLocal "Bounty Hunter has Friendly Players";
		};
		case "MIXED": {
			_markerNameZone setmarkerColorLocal _col_mixed;
			_markerNameDescription setmarkerColorLocal _col_mixed;
			_markerNameDescription setMarkerTextLocal "Bounty Hunter has Enemy and Friendly Players";
		};
	};
...
while {showmarkers} do {
    {
    	_unit = missionNamespace getVariable (_bountyStores select _forEachIndex);
		_friendlyCount = 0;
		_enemyCount = 0;
		{
